...
<div class="container-fluid py-4">
    ...
    <div class="row mb-3">
        <div class="col-12 d-flex justify-content-between align-items-center flex-wrap gap-2">
            <h1 class="h3 mb-0"><i class="fas fa-calendar-alt text-success me-2"></i>Mon calendrier</h1>
            <button class="btn btn-success" onclick="openCreateModal(new Date(), new Date(Date.now()+3600000), false)">
    ...
    <div class="row">
        ...
        <div class="col-12 col-lg-3 mb-4">
            <div class="card shadow mb-3">
                ...
                <div class="card-body small">
                    ...
                    <div class="mb-2 d-flex align-items-center">
                        <span class="legend-dot" style="background:<?= htmlspecialchars($cat['couleur']) ?>"></span>
                        <span>
                            <i class="<?= htmlspecialchars($cat['icone']) ?> me-1" style="color:<?= htmlspecialchars($cat['couleur']) ?>"></i>
            ...
            <div class="card shadow">
                ...
                <div class="card-body small">
                    <ul class="mb-0 ps-3">
                        <li><strong>Loyers</strong>, <strong>animations</strong> et <strong>rappels fiscaux</strong> sont générés automatiquement</li>
                        <li>Cliquez sur une date pour créer un RDV (médical, famille, autre)</li>
...
<div class="modal fade" id="eventModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header" style="background:linear-gradient(135deg,#6610f2,#4b0db3)">
            ...
            <div class="modal-body">
                ...
                <div class="row g-3">
                    <div class="col-12">
                        ...
                        <input type="text" class="form-control" id="eventTitle" placeholder="Ex: RDV cardiologue">
                    </div>
                    <div class="col-12 col-md-6">
                        ...
                        <select class="form-select" id="eventCategory">
                            <?php foreach ($categories as $cat): ?>
                                ...
                                <option value="<?= (int)$cat['id'] ?>" data-color="<?= htmlspecialchars($cat['couleur']) ?>"><?= htmlspecialchars($cat['nom']) ?></option>
                                <?php endif; ?>
                            <?php endforeach; ?>
                        </select>
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    calendar = new tui.Calendar('#calendar', {
        ...
        week:  { startDayOfWeek: 1, hourStart: 7, hourEnd: 22, daynames: ['Dim','Lun','Mar','Mer','Jeu','Ven','Sam'] },
        template: {
            allday: s => s.title,
    ...
    calendar.on('beforeUpdateSchedule', e => {
        ...
        if (schedule.raw && schedule.raw.autoGenere) return;
        if (changes && (changes.start || changes.end)) {
            calendar.updateSchedule(schedule.id, schedule.calendarId, changes);
            fetch(BASE + '/resident/calendarAjax/move', {
                method: 'POST', headers: jsonHeaders(),
                body: JSON.stringify({
                    id: schedule.id,
                    start: formatISO(changes.start || schedule.start),
                    end:   formatISO(changes.end   || schedule.end)
                })
            });
        }
    });
    ...
    document.getElementById('calendar').addEventListener('dblclick', e => {
        ...
        for (const cat of CATEGORIES) {
            ...
            if (sched) {
                if (sched.raw && sched.raw.autoGenere) return;
                openEditModal(sched);
                return;
            }
        }
    ...
    document.getElementById('deleteEventBtn').addEventListener('click', () => {
        ...
        fetch(BASE + '/resident/calendarAjax/delete', {
            method: 'POST', headers: jsonHeaders(),
...
function reloadEvents() {
    ...
    fetch(BASE + '/resident/calendarAjax/getEvents?start=' + fmt(start) + '&end=' + fmt(end))
        .then(r => r.json())
        .then(events => {
            calendar.clear();
            if (events.length) {
...
function openEditModal(sched) {
    ...
    document.getElementById('eventCategory').value = (sched.raw && sched.raw.categoryId) || '';
    ...
    document.getElementById('eventDescription').value = (sched.raw && sched.raw.description) || '';
    modal.show();
}
...
function saveEvent() {
    ...
    fetch(BASE + '/resident/calendarAjax/save', {
        method: 'POST', headers: jsonHeaders(),
