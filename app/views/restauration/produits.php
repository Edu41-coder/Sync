...
<div class="container-fluid py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2><i class="fas fa-boxes-stacked me-2 text-warning"></i>Catalogue Produits</h2>
        <button class="btn btn-warning" data-bs-toggle="modal" data-bs-target="#modalProduit"><i class="fas fa-plus me-2"></i>Nouveau produit</button>
    </div>
    ...
    <div class="card shadow-sm">
        <div class="card-header d-flex justify-content-between flex-wrap gap-2">
            <select id="filterCategorie" class="form-select form-select-sm" style="width:auto">
                ...
                <?php foreach (['fruits_legumes'=>'Fruits & Légumes','viandes'=>'Viandes','poissons'=>'Poissons','laitier'=>'Laitier','boulangerie'=>'Boulangerie','epicerie_seche'=>'Épicerie sèche','boissons'=>'Boissons','surgeles'=>'Surgelés','condiments'=>'Condiments','non_alimentaire'=>'Non alimentaire'] as $k=>$v): ?>
        ...
        <div class="card-body p-0">
            <table class="table table-hover mb-0" id="produitsTable">
                <thead><tr><th>Produit</th><th>Catégorie</th><th>Unité</th><th>Marque</th><th>Fournisseur</th><th class="text-end">Prix réf.</th><th class="text-center">Statut</th><th class="text-center">Actions</th></tr></thead>
                <tbody>
                    <?php if (empty($produits)): ?>
                    <tr><td colspan="8" class="text-center text-muted py-4">Aucun produit. Commencez par en créer.</td></tr>
...
<div class="modal fade" id="modalProduit" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form method="POST" action="<?= BASE_URL ?>/restauration/produits/create">
                <input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token'] ?? '' ?>">
                <div class="modal-header"><h5 class="modal-title"><i class="fas fa-plus me-2"></i>Nouveau Produit</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-3"><label class="form-label">Unité <span class="text-danger">*</span></label>
                            <select name="unite" class="form-select" required>
                                <?php foreach (['kg','g','litre','cl','unite','barquette','carton','sachet','boite','bouteille'] as $u): ?><option value="<?= $u ?>"><?= $u ?></option><?php endforeach; ?>
                            </select></div>
...
<script>new DataTableWithPagination('produitsTable', { rowsPerPage: 20, searchInputId: 'searchInput', filters: [{ id: 'filterCategorie', column: 1 }], paginationId: 'paginationProd', infoId: 'tableInfoProd' });</script>
