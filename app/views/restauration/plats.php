            ...
            <table class="table table-hover mb-0" id="platsTable">
                ...
                <tbody>
                    ...
                    <tr class="<?= $p['actif'] ? '' : 'opacity-50' ?>">
                        ...
                        <td class="text-end">
                            <a href="<?= BASE_URL ?>/restauration/plats/edit/<?= $p['id'] ?>" class="btn btn-sm btn-outline-primary" title="Modifier" data-bs-toggle="tooltip"><i class="fas fa-edit"></i></a>
                            <?php if ($p['actif']): ?>
                            <form method="POST" action="<?= BASE_URL ?>/restauration/plats/delete/<?= $p['id'] ?>" class="d-inline" onsubmit="return confirm('Désactiver ce plat ?')">
