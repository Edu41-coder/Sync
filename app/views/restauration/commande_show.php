        ...
        <div class="col-lg-10">
            <div class="card shadow-sm">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="mb-0"><i class="fas fa-truck me-2"></i>Commande <?= htmlspecialchars($commande['numero_commande']) ?></h5>
                ...
                <div class="card-body">
                    ...
                    <?php if ($showReception): ?><form method="POST" action="<?= BASE_URL ?>/restauration/commandes/receptionner/<?= $commande['id'] ?>"><input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token'] ?? '' ?>"><?php endif; ?>
                    ...
                    <table class="table">
                        ...
                        <tfoot>
                            ...
                            <tr><td colspan="<?= ($showReception || $commande['statut'] === 'livree') ? 4 : 3 ?>" class="text-end">TVA</td><td class="text-end"><?= number_format($commande['montant_tva'],2,',',' ') ?> &euro;</td></tr>
                ...
                <div class="card-footer d-flex justify-content-between">
                    <a href="<?= BASE_URL ?>/restauration/commandes" class="btn btn-secondary"><i class="fas fa-arrow-left me-2"></i>Retour</a>
                    <?php if ($isManager): ?>
                    <div class="d-flex gap-2">
                        <?php if ($commande['statut'] === 'brouillon'): ?>
                        <a href="<?= BASE_URL ?>/restauration/commandes/envoyer/<?= $commande['id'] ?>" class="btn btn-primary" onclick="return confirm('Marquer comme envoyée ?')"><i class="fas fa-paper-plane me-2"></i>Envoyer</a>
                        <a href="<?= BASE_URL ?>/restauration/commandes/delete/<?= $commande['id'] ?>" class="btn btn-danger" onclick="return confirm('Supprimer ?')"><i class="fas fa-trash me-2"></i>Supprimer</a>
                        <?php endif; ?>
