...
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
        <h2 class="mb-0"><i class="fas fa-truck-loading me-2 text-success"></i>Fournisseurs Jardinage</h2>
        <?php if ($selectedResidence && !empty($disponibles)): ?>
        <button class="btn btn-success" data-bs-toggle="modal" data-bs-target="#modalLier">
    ...
    <div class="alert alert-info"><i class="fas fa-info-circle me-2"></i>Sélectionnez une résidence pour gérer ses fournisseurs jardinage.</div>
    <?php else: ?>

    <?php if (empty($fournisseurs)): ?>
    <div class="alert alert-warning">
        <i class="fas fa-exclamation-circle me-2"></i>Aucun fournisseur lié à cette résidence.
        <?= !empty($disponibles) ? 'Utilisez le bouton <strong>« Lier un fournisseur »</strong> ci-dessus pour en ajouter.' : 'Aucun fournisseur disponible dans la base — créez-en via le module Admin.' ?>
    </div>
    <?php else: ?>
                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                            ...
                            <?php if ($f['ville']): ?><br><small class="text-muted"><i class="fas fa-map-marker-alt me-1"></i><?= htmlspecialchars($f['ville']) ?></small><?php endif; ?>
                        </td>
                        <td>
                            <?php if ($f['contact_local']): ?>
                                <strong><?= htmlspecialchars($f['contact_local']) ?></strong> <small class="text-muted">(local)</small>
                            <?php elseif ($f['contact_nom']): ?>
                                <small><?= htmlspecialchars($f['contact_nom']) ?> <span class="text-muted">(global)</span></small>
                            <?php else: ?>—<?php endif; ?>
                            <?php if ($f['email']): ?><br><small><a href="mailto:<?= htmlspecialchars($f['email']) ?>" class="text-decoration-none"><?= htmlspecialchars($f['email']) ?></a></small><?php endif; ?>
                        </td>
                        <td>
                            <?php $tel = $f['telephone_local'] ?: $f['telephone']; ?>
                            <?php if ($tel): ?><small><i class="fas fa-phone me-1"></i><a href="tel:<?= htmlspecialchars($tel) ?>" class="text-decoration-none"><?= htmlspecialchars($tel) ?></a></small><?php if ($f['telephone_local']): ?><br><small class="text-muted">(local)</small><?php endif; ?><?php else: ?>—<?php endif; ?>
                        </td>
                        <td>
                            <?php if ($f['jour_livraison']): ?><span class="badge bg-secondary"><?= htmlspecialchars($f['jour_livraison']) ?></span><?php endif; ?>
                            <?php if ($f['delai_livraison_jours']): ?><br><small class="text-muted"><?= (int)$f['delai_livraison_jours'] ?> j délai</small><?php endif; ?>
                            <?php if (!$f['jour_livraison'] && !$f['delai_livraison_jours']): ?><small class="text-muted">—</small><?php endif; ?>
                        </td>
                        <td class="small text-muted" style="max-width:240px"><?= htmlspecialchars(mb_strimwidth($f['pivot_notes'] ?? '', 0, 80, '...')) ?></td>
                        <td class="text-end">
                            ...
                            <form method="GET" action="<?= BASE_URL ?>/jardinage/fournisseurs/delier/<?= (int)$f['pivot_id'] ?>" class="d-inline" onsubmit="return confirm('Délier ce fournisseur de la résidence ? (statut → inactif)')">
...
<div class="modal fade" id="modalLier" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form method="POST" action="<?= BASE_URL ?>/jardinage/fournisseurs/lier">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="fournisseur_id" class="form-select" required>
                            ...
                            <option value="<?= (int)$d['id'] ?>"><?= htmlspecialchars($d['nom']) ?><?= $d['type_service'] ? ' — ' . htmlspecialchars($d['type_service']) : '' ?></option>
                            <?php endforeach; ?>
                        </select>
                        <small class="text-muted">Seuls les fournisseurs pas encore liés à cette résidence apparaissent.</small>
                    </div>

                    <h6 class="text-muted text-uppercase small mt-4 mb-2">Informations locales (optionnel)</h6>
                    <p class="text-muted small">Ces champs surchargent les infos globales du fournisseur pour cette résidence uniquement.</p>
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="number" min="0" name="delai_livraison_jours" class="form-control" placeholder="Ex : 3">
                        </div>
                        <div class="col-12">
                            <label class="form-label">Notes</label>
                            <textarea name="notes" class="form-control" rows="2" placeholder="Conditions tarifaires locales, remarques…"></textarea>
                        </div>
                    </div>
                </div>
        ...
        <div class="modal-content">
            <form method="POST" id="formEditLien">
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title"><i class="fas fa-edit me-2"></i>Modifier le lien — <span id="editLienFournNom"></span></h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token'] ?? '' ?>">
...
<script>
function editLien(f) {
    document.getElementById('formEditLien').action = '<?= BASE_URL ?>/jardinage/fournisseurs/update/' + f.pivot_id;
    document.getElementById('editLienFournNom').textContent = f.nom;
    ...
    document.getElementById('editNotes').value = f.pivot_notes || '';
