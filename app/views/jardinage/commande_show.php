...
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
        <div>
            <h2 class="mb-0">
                <i class="fas fa-file-invoice me-2 text-success"></i><?= htmlspecialchars($commande['numero_commande']) ?>
        ...
        <div class="d-flex gap-2 flex-wrap">
            <a href="<?= BASE_URL ?>/jardinage/commandes" class="btn btn-outline-secondary"><i class="fas fa-arrow-left me-1"></i>Retour</a>
            <?php if ($canEnvoyer): ?>
            <a href="<?= BASE_URL ?>/jardinage/commandes/envoyer/<?= (int)$commande['id'] ?>" class="btn btn-info" onclick="return confirm('Marquer cette commande comme envoyée au fournisseur ?')">
            ...
            <?php endif; ?>
            <?php if ($canFacturer): ?>
            <a href="<?= BASE_URL ?>/jardinage/commandes/facturer/<?= (int)$commande['id'] ?>" class="btn btn-primary" onclick="return confirm('Marquer cette commande comme facturée ?')">
                <i class="fas fa-file-invoice-dollar me-1"></i>Facturer
            </a>
            <?php endif; ?>
            <?php if ($canAnnuler): ?>
            <a href="<?= BASE_URL ?>/jardinage/commandes/delete/<?= (int)$commande['id'] ?>" class="btn btn-outline-danger" onclick="return confirm('<?= $commande['statut'] === 'brouillon' ? 'Supprimer ce brouillon' : 'Annuler cette commande' ?> ?')">
        ...
        <div class="col-md-6">
            <div class="card shadow-sm h-100">
                ...
                <div class="card-body">
                    ...
                    <?php if ($commande['fournisseur_adresse']): ?><small class="text-muted"><?= htmlspecialchars($commande['fournisseur_adresse']) ?><br><?= htmlspecialchars($commande['fournisseur_cp']) ?> <?= htmlspecialchars($commande['fournisseur_ville']) ?></small><br><?php endif; ?>
                    <?php if ($commande['fournisseur_telephone']): ?><small><i class="fas fa-phone me-1"></i><?= htmlspecialchars($commande['fournisseur_telephone']) ?></small><br><?php endif; ?>
                    <?php if ($commande['fournisseur_email']): ?><small><i class="fas fa-envelope me-1"></i><a href="mailto:<?= htmlspecialchars($commande['fournisseur_email']) ?>"><?= htmlspecialchars($commande['fournisseur_email']) ?></a></small><?php endif; ?>
                </div>
            </div>
        </div>
        <div class="col-md-6">
            <div class="card shadow-sm h-100">
                ...
                <div class="card-body">
                    <dl class="row mb-0 small">
                        ...
                        <dt class="col-5">Créée par</dt><dd class="col-7"><?= $commande['created_by_prenom'] ? htmlspecialchars($commande['created_by_prenom'] . ' ' . $commande['created_by_nom']) : '—' ?></dd>
        ...
        <div class="card-body p-0">
            <table class="table mb-0">
                ...
                <tfoot class="table-light">
                    ...
                    <tr><td colspan="6" class="text-end fs-5"><strong>Total TTC :</strong></td><td class="text-end fs-5 text-success"><strong><?= number_format($commande['montant_total_ttc'], 2, ',', ' ') ?> €</strong></td></tr>
                </tfoot>
            </table>
        </div>
    </div>
...
<div class="modal fade" id="modalReception" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form method="POST" action="<?= BASE_URL ?>/jardinage/commandes/receptionner/<?= (int)$commande['id'] ?>">
